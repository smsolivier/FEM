...
void Picard::solve(vector<double>& a_sol, FEGrid& a_grid, 
	const Materials& a_materials, LinearSolver& a_solver, 
	double a_tol, int a_maxiter) {

	CH_TIMERS("picard iterations"); 

	// initialize solution 
	a_sol.resize(a_grid.getFields().size()); 
	...
	for (i=0; i<a_maxiter; i++) {
		NSOp * ns = new NSOp(a_grid, a_materials);
		ns->buildLHS(psol); 
		...
		if (m_verbose > 1) {
			...
			a_grid.writeFields(a_sol, fname.c_str()); 
		}
